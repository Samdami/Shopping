...
def home():
    ...
    products = (
        Addproduct.query.filter(Addproduct.stock > 0)
...
def updatecat(id):
    ...
    return render_template(
        "products/addbrand.html", title="Update cat", updatecat=updatecat
    )
    ...
    if request.method == "POST":
        ...
        if request.files.get("image_1"):
            try:
                os.unlink(
                    os.path.join(
                        current_app.root_path, "static/images/" + product.image_1
                    )
                )
        ...
        if request.files.get("image_2"):
            try:
                os.unlink(
                    os.path.join(
                        current_app.root_path, "static/images/" + product.image_2
                    )
                )
        ...
        if request.files.get("image_3"):
            try:
                os.unlink(
                    os.path.join(
                        current_app.root_path, "static/images/" + product.image_3
                    )
                )
